        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add{{ $activeMenu->access }}Form">
                    <div class="mb-3">
                        <label for="komponen" class="form-label">Komponen Perkembangan</label>
                        <input type="text" class="form-control" id="komponen" name="komponen" required>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="updateForm">
                    ...
                    <div class="mb-3">
                        <label for="updateKomponenPerkembangan" class="form-label">Komponen Perkembangan</label>
                        <input type="text" class="form-control" id="updateKomponenPerkembangan" name="komponen" required>
                    </div>
                    <div class="mb-3">
...
<script>
    function load_this_data(){
        loadTabelData("list-data", "{{route('load_komponen_perkembangan')}}", ['No', 'Komponen Perkembangan','Urutan','Aksi']);
    }
    $(document).ready(function () {
        ...
        $('#add{{ $activeMenu->access }}Form').on('submit', function (e) {
            ...
            $.ajax({
                url: '{{ route("komponenperkembangan.store") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
        ...
        $('#updateForm').on('submit', function (e) {
            ...
            $.ajax({
                url: '{{ route("komponenperkembangan.update") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: $('#updateId').val(),
                    komponen: $('#updateKomponenPerkembangan').val(),
                },
                success: function (response) {
                    Swal.fire({
                        icon    : 'success',
    ...
    function update_form(id) {
        $.ajax({
            url: `/komponen-perkembangan/edit/${id}`,
            type: 'GET',
    ...
    function delete_form(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/komponen-perkembangan/delete/${id}`,
                    type: 'DELETE',
                    headers: {
