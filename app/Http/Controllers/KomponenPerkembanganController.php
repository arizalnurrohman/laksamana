...
class KomponenPerkembanganController extends Controller
{
    ...
    {
        $komponen_intervensi = [];
        return view('komponen-perkembangan.index', compact('komponen_intervensi'));
    }
    public function store(Request $request)
    {
        ...
        $lastSort = KomponenPerkembangan::max('sort') ?? 0;

        KomponenPerkembangan::create([
    ...
    {
        $komponen = KomponenPerkembangan::findOrFail($request->id);
    ...
    {
        $request->validate([
            'id' => 'required|exists:laksa_ms_komponen_perkembangan,id',
            'komponen' => 'required|string|max:255',
        ]);
    ...
    public function destroy($id)
    {
        try {
            // Cari dan hapus data
            $komponen = KomponenPerkembangan::findOrFail($id);
            $komponen->delete();
    ...
    public function load_data()
    {
        ...
        foreach ($komponen as $val) {
            $data[$no]['No']            =($no+1);
            $data[$no]['Komponen Perkembangan']   =$val->komponen;
