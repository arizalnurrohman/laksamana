...
class LaporanTerminasiController extends Controller
{
    ...
    //     $this->error    =array();
    //     $this->success  =false;
        $this->status_usulan=['2518e902-e601-11ef-bfa8-244bfebc0c45'];
    }
    public function index()
    {
        return view('laporan_terminasi.index');
    }

    public function load_laporan(){
        $sub_child = Residensial::select("laksa_tr_layanan.id as residensial_id","laksa_tr_layanan.*","laksa_ms_ppks.*","laksa_ms_sumber_rujukan.*","laksa_ms_pendamping_sosial.*","laksa_ms_pegawai.*","laksa_ms_status.*","laksa_ms_status.*","laksa_tr_status.*","laksa_tr_status.created_at as tgl_status","laksa_tr_laporan_terminasi.*");
        ...
        $sub_child = $sub_child->leftJoin('laksa_tr_laporan_terminasi', 'laksa_tr_layanan.id', '=', 'laksa_tr_laporan_terminasi.layanan_id');
        $sub_child = $sub_child->whereIn("laksa_tr_layanan.status_id",$this->status_usulan);
        ...
        foreach ($sub_child as $val) {
            ...
            $data[$no]['Tanggal']           =date("d-m-Y H:i:s",strtotime($val->tgl_terminasi));
    ...
    public function get_laporanterminasi($id){
        $dokumen_la = LaporanTerminasi::where("layanan_id",$id)->first();
        // dd($dokumen_ba);
        $return['dokumen_ba']=asset('storage/' . $dokumen_la->dokumen_terminasi);
        return $return;
    }
}
