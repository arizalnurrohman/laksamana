...
class ListAspekController extends Controller
{
    ...
    {
        $liast_aspek = [];
        return view('aspek.index', compact('liast_aspek'));
    }
    ...
    {
        $aspek = Aspek::findOrFail($request->id);
    ...
    {
        $request->validate([
            'id' => 'required|exists:laksa_ms_aspek,id',
            'aspek' => 'required|string|max:255',
        ]);
    ...
    public function load_data()
    {
        ...
        $no=0;
        foreach ($aspek as $val) {
            $data[$no]['No']            =($no+1);
            $data[$no]['Aspek']         =$val->aspek;
