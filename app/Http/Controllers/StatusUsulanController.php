...
class StatusUsulanController extends Controller
{
    ...
    {
        $status = [];
        return view('status-usulan.index', compact('status'));
    }
    public function store(Request $request)
    {
        $request->validate([
            'status_usulan' => 'required|string|max:255',
        ]);

        $lastSort = StatusUsulan::max('sort') ?? 0;

        StatusUsulan::create([
            'id' => Str::uuid()->toString(),
            'status' => $request->status_usulan,
            'sort' => $lastSort + 1,
        ]);
    ...
    {
        $status = StatusUsulan::findOrFail($request->id);
    ...
    {
        $request->validate([
            'id' => 'required|exists:laksa_ms_status,id',
            'status_usulan' => 'required|string|max:255',
        ]);
    ...
    public function destroy($id)
    {
        try {
            // Cari dan hapus data
            $status = StatusUsulan::findOrFail($id);
            $status->delete();
    ...
    {
        $kategori = StatusUsulan::orderby("jenis_layanan","asc")->orderby("sort","asc")->get();
        $no=0;
        $data=array();
        foreach ($kategori as $val) {
            $data[$no]['No']        =($no+1);
            $data[$no]['Status']  =$val->status;
