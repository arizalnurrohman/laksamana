...
class LaporanPerkembanganController extends Controller
{
    ...
    public function load_laporan(){
        ...
        $sub_child = $sub_child->leftJoin('laksa_tr_rehabilitasi', 'laksa_tr_layanan.id', '=', 'laksa_tr_rehabilitasi.residensial_id');
        $sub_child = $sub_child->where("laksa_tr_rehabilitasi.laporan_rehabilitasi","!=",null);
        ...
        foreach ($sub_child as $val) {
            ...
            $data[$no]['Aksi']              ='<div class="btn-group" role="group" aria-label="Group Aksi">
                                                <button class="btn btn-sm btn-icon btn-info" Onclick="dokumen_perkembangan(\''.$val->residensial_id.'\')" title="Lihat Dokumen Rehabilitasi">
    ...
    public function get_laporanperkembangan($id){
        $dokumen_rehabilitasi = Rehabilitasi::where("residensial_id",$id)->first();
        // dd($dokumen_ba);
        $return['dokumen_ba']=asset('storage/' . $dokumen_rehabilitasi->laporan_rehabilitasi);
        return $return;
    }
}
