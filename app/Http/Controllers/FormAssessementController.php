...
class FormAssessementController extends Controller
{
    ...
    {
        $form_assessement = [];
        return view('form-assessement.index', compact('form_assessement'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $lastSort = FormAssessment::max('sort') ?? 0;

        FormAssessment::create([
            'id' => Str::uuid()->toString(),
            'assessment' => $request->kategori,
            'sort' => $lastSort + 1,
        ]);
    ...
    {
        $kategori = FormAssessment::orderby("sort","asc")->get();
        $no=0;
        $data=array();
        foreach ($kategori as $val) {
            $data[$no]['No']        =($no+1);
            $data[$no]['Assessment']  =$val->assessment;
    ...
    {
        $kategori_sub = FormAssessmentSub::select("laksa_ms_form_assessment_sub.*")->where("form_assessment_id","=",$id)->whereNull("parent_id")->orderby("sort","asc");
        $kategori_sub = $kategori_sub->addSelect([
            'total_child' => FormAssessmentSub::selectRaw('COUNT(*)')
                ->whereColumn('parent_id', '=', 'laksa_ms_form_assessment_sub.id')
                ->take(1)
        ]);
        ...
        foreach ($kategori_sub as $val) {
            $data[$no]['No']        =($no+1);
            $data[$no]['Sub Assessement']  =$val->sub_kategori_assessment.($val->total_child >0 ? " [".$val->total_child."]" : "");
            $data[$no]['Aksi']      ='<div class="btn-group" role="group" aria-label="Group Aksi">
    ...
    public function get_assessment($id){
        $data = FormAssessment::where('id', $id)->get();
        return response()->json($data);
    }
    ...
    public function sub_store(Request $request)
    {
        ...
        $request->validate([
            'form_assessment_id' => 'required',
            'kategori_sub' => 'required|string|max:255',
        ]);

        $lastSort = FormAssessmentSub::where("parent_id","=",$request->parent_id)->max('sort') ?? 0;

        if($request->sub_id){
            $subkategori = FormAssessmentSub::findOrFail($request->sub_id);
            $subkategori->update([
                'sub_kategori_assessment' => $request->kategori_sub,
            ]);
            return response()->json(['message' => 'Data berhasil diperbarui.']);
        }else{
            FormAssessmentSub::create([
                'id'                => Str::uuid()->toString(),
                'form_assessment_id'=> $request->form_assessment_id,
    ...
    public function sub_child_assessement($id){
        $data = FormAssessmentSub::where('id', $id)->get();
        return response()->json($data);
    }

    public function load_sub_child_list_assessment($id){
        $sub_child = FormAssessmentSub::where('parent_id', $id)->orderby("sort","asc")->get();
        $data = array();
        $no=0;
        foreach ($sub_child as $val) {
            $data[$no]['No']        =($no+1);
            $data[$no]['Pilihan']   =$val->sub_kategori_assessment;
            $data[$no]['Aksi']      ='<div class="btn-group" role="group" aria-label="Group Aksi">
    ...
    public function destroy_sub_child($id)
    {
        try {
            // Cari dan hapus data
            $persyaratan = FormAssessmentSub::findOrFail($id);
            $persyaratan->delete();
