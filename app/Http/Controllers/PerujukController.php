...
class PerujukController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($this->success) {
            $response = [
                'status' => 'success',
                'message' => "Data Perujuk berhasil disimpan.",
            ];
        }
    ...
    public function edit(Request $request)
    {
        ...
        $get_data   = Perujuk::findOrFail($request->id);
        if($get_data){
            $return['success']  ='ok';
    ...
    {
        $perujuk = Perujuk::orderby("nama_perujuk","asc")->get();
        $no=0;
        $data=[];
        foreach ($perujuk as $val) {
            $data[$no]['No']            =($no+1);
            $data[$no]['Perujuk']       ="<strong>".$val->nip_nrp."</strong><br>".$val->nama_perujuk;
            $data[$no]['Pangkat']       =$val->pangkat_jabatan;
            $data[$no]['Instansi']      =$val->instansi_perujuk."<br>".$val->alamat_kantor."<br>".$val->telp_kantor;
